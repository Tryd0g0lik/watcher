...
export default function clockFC(boxFC: ({ height, width }: Sizes) => React.JSX.Element): () => void {
  // const height: 480;
  // const width: 480;
  function canvasies(): void {
    const canvasHTML = boxFC({ height: 480, width: 480 }) as unknown as HTMLCanvasElement;
    const contextHTML: CanvasRenderingContext2D | null = canvasHTML.getContext("2d");
    if (contextHTML !== null) {
